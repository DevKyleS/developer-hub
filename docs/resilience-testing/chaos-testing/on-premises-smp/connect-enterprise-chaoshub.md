...
:::warning Legacy Infrastructure Only
This documentation applies **only to legacy chaos infrastructure** (versions prior to SMP 0.24.x). 

For current SMP versions, Enterprise ChaosHub is built into the chaos-manager and does not require GitHub connectivity. For the latest information, refer to the [SMP Release Notes](/release-notes/self-managed-enterprise-edition).
:::

This topic describes the method to connect Enterprise ChaosHub (https://github.com/) in an air-gapped setup via proxy for legacy chaos infrastructure.

If you don't have access to [GitHub](https://github.com/), you will not be able to deploy Harness SMP in an air-gapped cluster completely with legacy infrastructure. As a consequence, the Enterprise ChaosHub will be in a `DISCONNECTED` state.
